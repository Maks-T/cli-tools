...
Этот скрипт использует предзаполненный идеальный шаблон настроек.
На каждом шаге он предлагает вам текущее значение по умолчанию.

Необходимые библиотеки: pip install questionary
Запуск: python config_wizard.py
"""
...
try:
    ...
    print("Выполните: pip install questionary")
    sys.exit(1)

OUTPUT_FILE = "import_settings.json"

# Базовый конфиг (Шаблон)
DEFAULT_SETTINGS = {
  "languages": ["ru", "en"],
  "channels": {
  ...
  "metadata_schemas": {
    "attribute": [
      ...
      {"key": "is_multiple", "type": "boolean", "label": {"ru": "Множественный выбор", "en": "Multiple choice"}, "width": 1, "is_system": True, "default": True},
      {"key": "filter_type", "type": "select", "label": {"ru": "Вид фильтра", "en": "Filter UI type"}, "width": 1, "is_system": True, "default": "checkbox", "options": {
    ...
    "complex_dictionary": [
      {"key": "is_public", "type": "boolean", "label": {"ru": "Опубликовано", "en": "Published"}, "width": 1, "is_system": True, "default": True},
      {"key": "is_settings_public", "type": "boolean", "label": {"ru": "Настройки публичны", "en": "Settings are public"}, "width": 1, "is_system": True, "default": True},
      {"key": "contains_prices", "type": "boolean", "label": {"ru": "Содержит цены", "en": "Contains prices"}, "width": 2, "is_system": True, "default": False}
    ],
    "family": [
      {"key": "is_public", "type": "boolean", "label": {"ru": "Опубликовано", "en": "Published"}, "width": 1, "is_system": True, "default": True},
    ...
    "product_type": [
      {"key": "is_public", "type": "boolean", "label": {"ru": "Опубликовано", "en": "Published"}, "width": 1, "is_system": True, "default": True},
      {"key": "is_settings_public", "type": "boolean", "label": {"ru": "Настройки публичны", "en": "Settings are public"}, "width": 1, "is_system": True, "default": True},
      {"key": "pricing_mode", "type": "select", "label": {"ru": "Режим ценообразования", "en": "Pricing mode"}, "width": 2, "is_system": True, "default": "manual", "options": {
          "manual": {"ru": "Вручную", "en": "Manual"},
          "complex_dictionary": {"ru": "Из справочника", "en": "From dict"}
      }}
    ],
    "product": [
      {"key": "is_public", "type": "boolean", "label": {"ru": "Опубликовано", "en": "Published"}, "width": 1, "is_system": True, "default": True},
...
class Wizard:
    ...
    def build_select_options(self, active_langs: list) -> dict:
        ...
        while True:
            ...
            if not val.strip():
                print(f"{Colors.WARNING}Value не может быть пустым!{Colors.ENDC}")
                continue
            ...
            for lang in active_langs[1:]:
                label_dict[lang] = label_primary

            options[val] = label_dict
            if not check_exit(questionary.confirm("Добавить еще опцию?").ask()):
                break
        return options
    ...
    def run(self):
        ...
        # Языки
        langs_str = check_exit(questionary.text(
            "Языки системы (через запятую):",
            default=", ".join(self.config["languages"])
        ).ask())
        self.config["languages"] = [l.strip() for l in langs_str.split(",") if l.strip()]

        # Каналы
        ch_str = check_exit(questionary.text(
            "Каналы продаж (через запятую):",
            default=", ".join(self.config["channels"].keys())
        ).ask())
        ...
        for ch in channels:
            ...
            is_pub = check_exit(questionary.confirm(
                f"Канал '{ch}': делать новые элементы публичными по умолчанию?",
                default=default_pub
            ).ask())
            new_channels[ch] = {"is_public_default": is_pub}
        self.config["channels"] = new_channels

        # Мета-схемы сущностей
        print(f"\n{Colors.BOLD}{Colors.BLUE}Настройка мета-схем (Полей){Colors.ENDC}")
        primary_lang = self.config["languages"][0]

        for entity, fields in list(self.config["metadata_schemas"].items()):
            edit = check_exit(questionary.confirm(
                f"Сущность '{entity}' (полей: {len(fields)}). Настроить/добавить поля?",
                default=False
            ).ask())
            ...
            for f in fields:
                ...
                k = check_exit(questionary.text("Ключ (очистите строку, чтобы удалить поле):", default=f['key']).ask())
                if not k.strip():
                    print(f"{Colors.WARNING}Поле удалено.{Colors.ENDC}")
                    continue
                ...
                if isinstance(f['label'], dict):
                    f['label'][primary_lang] = new_lbl
                else:
                    f['label'] = {primary_lang: new_lbl}

                f['width'] = int(check_exit(questionary.select("Ширина (1-4):", choices=["1","2","3","4"], default=str(f.get('width', 1))).ask()))

                if f['type'] == 'select' and 'options' not in f:
                    f['options'] = self.build_select_options(self.config["languages"])
            ...
            while True:
                ...
                k = check_exit(questionary.text("Ключ нового поля (пусто для завершения с этой сущностью):").ask())
                if not k.strip():
                    break
                ...
                new_f = {
                    "key": k.strip(),
                    "type": t,
                    "label": {primary_lang: lbl},
                    "width": w,
                    "is_system": False
                }

                for lang in self.config["languages"][1:]:
                    new_f["label"][lang] = k.strip()

                if t == 'select':
                    new_f['options'] = self.build_select_options(self.config["languages"])

                new_fields.append(new_f)

            self.config["metadata_schemas"][entity] = new_fields
